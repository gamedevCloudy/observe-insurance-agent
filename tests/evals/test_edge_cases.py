...
from tests.evals.scenarios import EDGE_SCENARIOS


@pytest.mark.evals
def test_edge_cases(agent_graph, judge_client):
    async def _run():
        _recreate_llm()
        ...
        return await asyncio.gather(*[_limited(s) for s in EDGE_SCENARIOS])
    ...
    if failures:
        ...
        pytest.fail(f"{len(failures)}/{len(results)} edge case scenarios failed:\n{details}")
